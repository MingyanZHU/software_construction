...
public class OutputGraph {
  ...
  /**
   * Adapter of original outputGraph which you choose strategy you wanna to output graph.
   *
   * @param graph graph instance which you wanna to output.
   * @param fileName file path the output file will store.
   * @param outputStrategy OutputStrategy instance with file name of you wanna to using.
   * @throws IOException if there is no such file path.
   */
  // 可以选择输出策略的输出图函数
  public static void outputGraphStrategy(Graph graph, String fileName,
      OutputStrategy outputStrategy) throws IOException {
    out = outputStrategy;
    if (!out.getFilePath().equals(fileName)) {
  ...
  /**
   * Output the graph with the Lab3 demanding. 输出按照Lab3中的语法要求的图的信息至文件中永久保存
   *
  ...
  public static void outputGraph(Graph graph, String fileName) throws IOException {
    if (out == null || !fileName.equals(out.getFilePath())) {
      out = new BufferOutputStrategy(fileName);
    }
