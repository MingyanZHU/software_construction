...
public class MovieDirectorRelation extends UndirectedEdge {
  ...
  public boolean addVertices(List<Vertex> vertexList) throws InputFileAgainException {
    ...
    if (vertexList.size() != 2) {
      throw new IllegalEdgeParamsException("Add vertices into MovieDirectorRelation with "
          + vertexList.size() + " params");
    }
    if (!((vertexList.get(0) instanceof Director && vertexList.get(1) instanceof Movie)
        || (vertexList.get(0) instanceof Movie && vertexList.get(1) instanceof Director))) {
      throw new IllegalEdgeParamsException("Add vertices into MovieDirectorRelation with "
          + vertexList.get(0).getClass().getSimpleName() + " and "
          + vertexList.get(1).getClass().getSimpleName());
    }
    ...
    return super.addVertices(vertexList);
  }
