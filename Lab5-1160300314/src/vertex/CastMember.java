...
public abstract class CastMember extends Vertex {
  ...
  Safety for Rep Exposure:
      Other fields except label are modified by private. Clients can not access the field by other
      function except getter function and fillVertexInfo which is strictly limited.
   */
  ...
  /**
   * Fill the other fields in CastMember with a string array with two strings, which is age and
   * gender of the cast in order.
   *
  ...
  public void fillVertexInfo(String[] args)
    ...
    if (args.length != 2) {
      throw new IllegalParamsNumberException(
          "Fill vertices in Movie Graph with " + args.length + " param(s)");
    }
