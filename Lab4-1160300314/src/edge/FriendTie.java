...
public class FriendTie extends DirectedEdge {
  ...
  public FriendTie(String label, double weight) throws InputFileAgainException {
    ...
    if (weight <= 0 || weight > 1) {
      throw new IllegalEdgeParamsException("Weight of Friend Connection is illegal");
    }
  }
  ...
  public boolean addVertices(List<Vertex> vertices) throws InputFileAgainException {
    ...
    if (vertices.size() != 2) {
      throw new IllegalVertexParamsException(
          "Add vertices into FriendTie with " + vertices.size() + " params!");
    }
    assert vertices.size() == 2;
    if (vertices.get(0).equals(vertices.get(1))) {
      throw new IllegalEdgeParamsException("FriendTie can not be loop!");
    }
    boolean answer = true;
